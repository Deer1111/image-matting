...
if __name__=='__main__':
    ...
    for i in range(1,120):
        pred_pha=cv2.imread(os.path.join(pred_path,f'{i+1}.png'), cv2.IMREAD_GRAYSCALE).astype(np.float32) / 255
        true_pha=cv2.imread(os.path.join(true_path,f'{i+1}.png'), cv2.IMREAD_GRAYSCALE).astype(np.float32) / 255
